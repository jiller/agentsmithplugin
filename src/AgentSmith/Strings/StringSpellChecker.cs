...
namespace AgentSmith.Strings
{
    ...
    internal static class StringSpellChecker
    {
                                                       ...
                                                       ISolution solution, List<Regex> patternsToIgnore)
        {
            ...
            {
                if (SpellCheckUtil.ShouldSpellCheck(wordLexer.GetCurrTokenText()) &&
                    !spellCheckWithAmpersand(spellChecker, wordLexer.GetCurrTokenText(), true))
                {
                    
                    IClassMemberDeclaration containingElement =
                        token.GetContainingElement<IClassMemberDeclaration>(false);
                    if (containingElement == null ||
                        !IdentifierResolver.IsIdentifier(containingElement, solution, wordLexer.GetCurrTokenText()))
                    {
